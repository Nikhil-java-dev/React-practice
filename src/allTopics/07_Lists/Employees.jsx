const Employees = () => {
  ...
  return (
    <>
      ...
      {employeesData.users.map((users) => {
        return (
          <div key={users.id}>
            <h3>
              {users.firstName}
              {users.lastName} {users.maidenName} {users.age} {users.bloodGroup}
            </h3>
            <p>Hair Color: {users.hair.color}</p>
            <p>Hair Type: {users.hair.type}</p>
            <p>Address: {users.address.address}</p>
            <p>City: {users.address.city}</p>
            <p>
              State: {users.address.state}, Postalcode:{" "}
              {users.address.postalCode}, Country: {users.address.country}
            </p>
            <p>
              Coordinates - lat: {users.address.coordinates.lat}, lag:{" "}
              {users.address.coordinates.lng}
            </p>
            <p>
              Bank:- cardExpire: {users.bank.cardExpire}, cardNumber:{" "}
              {users.bank.cardNumber}, cardType: {users.bank.cardType},
              cardNumber: {users.bank.cardNumber}
            </p>
            <hr />
            <p>
              Company:- department: {users.company.department}, name:{" "}
              {users.company.name}, titel: {users.company.title}, address:{" "}
              {users.company.address.city}, State: {users.company.address.state}
              , Code: {users.company.address.stateCode}, Coordinates:-lat:{" "}
              {users.company.address.coordinates.lat}, lag:{" "}
              {users.company.address.coordinates.lng}
            </p>
          </div>
        );
      })}
